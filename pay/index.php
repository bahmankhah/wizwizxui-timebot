...
if (!isset($_GET['zarinpal']) && !isset($_GET['moneyaex']) && !isset($_GET['nowpayment']) && !isset($_GET['nextpay'])) {
...
if (mysqli_num_rows($payInfo) == 0) {
    ...
    } elseif (isset($_GET['moneyaex'])) {
        $Description = "خرید اشتراک";
        $CallbackURL = $botUrl . "pay/back.php?moneyaex&hash_id=$hash_id";

        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://om.wibitpay.com/api/v1.0/Pay/PayRequest',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
            ...
            CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
            CURLOPT_CUSTOMREQUEST => 'POST',
            CURLOPT_POSTFIELDS => 'AcceptorId=' . $myValues['acceptorId'] . '&' . $myValues['acceptorPassword'] . '&Amount=' . $amount . '&OrderId=' . $orderId . '&PaymentId' . $hash_id . '&Cellphone' . $userInfo['phone']  . '&Callback=' . $CallbackURL,
        ));
        $response = curl_exec($curl);
        curl_close($curl);
        $response = json_decode($response);
        if ($response->successful == true) {
            $sql = "UPDATE `pays` SET `pay_token` = ? AND `pay_currency` = ? WHERE `hash_id` = ?";
            $stmt = $connection->prepare($sql);
            $stmt->bind_param("sis", $response->data->token, $response->data->currency, $hash_id);
            $stmt->execute();
            $stmt->close();
            $keyboard = [['text'=>$buttonValues['i_paid'], 'callback_data'=> "checkPaidWithMoneyaex".$hash_id],['text'=>$buttonValues['cancel'], 'callback_data'=> "mainMenu"]];

    
            $keys = json_encode(['inline_keyboard'=>$keyboard]);
            sendMessage("در صورت پرداخت دکمه را بزنید", $keys);
        } else {
            showForm("تراکنش با خطا مواجه شده است");
        }
    }
