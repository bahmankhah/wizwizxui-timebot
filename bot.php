...
if ($userInfo['step'] == "increaseMyWallet" && $text != $buttonValues['cancel']) {
    ...
    $keyboard = array();
    if ($botState['moneyaex'] == "on") {
        $keyboard[] = [['text' => $buttonValues['moneyaex'],  'url' => "pay/?nowpayment&hash_id=" . $hash_id]];
    }
...
if (preg_match('/checkPaidWithMoneyaex/', $data, $match) and $text != $buttonValues['cancel']) {
    $hid = $match[1];
    $stmt = $connection->prepare("SELECT * FROM `pays` WHERE `hash_id` = ?");
    $stmt->bind_param('s', $hid);
    $stmt->execute();
    $payRow = $stmt->get_result()->fetch_assoc();
    $token = $payRow['pay_token'];

    $curl = curl_init();
    curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://om.wibitpay.com/api/v1.0/Pay/Inquiry',
        ...
        CURLOPT_POSTFIELDS => 'AcceptorId=' . $my'' . '&AcceptorPassword' . '123456789' . '&Token=' . $token,
    ));
    ...
    if ($response->successful) {
        if ($response->data->status == 1) {
            doAction($payRow['id'], 'moneyaex');
        } else {
            showForm("پرداخت انجام نشده", $response->data->statusName);
        }
    } else {
        showForm("مشکلی پیش آمد...", 'خطا!');
    }
    exit;
}

if (preg_match('/increaseWalletWithCartToCart/', $data, $match) and $text != $buttonValues['cancel']) {
    $hid = $match[1];
    $stmt = $connection->prepare("SELECT `pay_token` FROM `pays` WHERE `hash_id` = '$hid' ");
    $stmt->execute();
    $token = $stmt->get_result()->fetch_assoc()['pay_token'];
    ...
    curl_setopt_array($curl, array(
        ...
        CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
        CURLOPT_CUSTOMREQUEST => 'POST',
        CURLOPT_POSTFIELDS => 'AcceptorId=' . $paymentKeys['nextpay'] . '&AcceptorPassword' . '' . '&Amount=' . $amount . '&token=' . $token,
    ));
    $response = curl_exec($curl);
    curl_close($curl);
    $response = json_decode($response);
...
if ((preg_match('/^discountCustomPlanDay(\d+)/', $userInfo['step'], $match) || preg_match('/enterCustomPlanName(\d+)_(\d+)_(\d+)_(\d+)/', $userInfo['step'], $match)) && ($botState['sellState'] == "on" || $from_id == $admin) && $text != $buttonValues['cancel']) {
    ...
    if (!preg_match('/^discountCustomPlanDay/', $userInfo['step'])) {
        ...
        if ($userInfo['is_agent'] == true && $userInfo['temp'] == "agentBuy") {
            $gbPrice -= ($gbPrice * $userInfo['discount_percent'] / 100);
            $dayPrice -= ($dayPrice * $userInfo['discount_percent'] / 100);
        }
